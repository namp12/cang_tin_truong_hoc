...
export const SystemTesterPage: React.FC = () => {
  ...
  const [modules, setModules] = useState<ModuleTest[]>([
    ...
    },
    {
      ...
      storageKey: 'dnu_canteen_finance_ledger_v2',
      ...
      latencyMs: 4,
      lastTestedAt: 'Vừa xong',
      itemCount: dnuStore.getFinanceTransactions().length,
      sampleDataSummary: `Số dư quỹ: ${formatCurrency(dnuStore.getFinanceSummary().netBalance)} (${dnuStore.getFinanceTransactions().length} chứng từ thu/chi)`,
    },
    {
      id: 'promotions',
    ...
    },
    {
      ...
      latencyMs: 5,
      lastTestedAt: 'Vừa xong',
      itemCount: dnuStore.getReviews().length,
      sampleDataSummary: `Điểm trung bình: ${dnuStore.getReviewStats().avgRating}/5.0 ⭐ (${dnuStore.getReviews().length} đánh giá thực tế)`,
    },
    {
      id: 'reports',
