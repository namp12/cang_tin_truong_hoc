...
export const StudentCartPage: React.FC = () => {
  ...
  const [paymentMethod, setPaymentMethod] = useState<'DNUPAY' | 'QRMOMO' | 'CASH'>('DNUPAY');
  ...
  const handleCheckout = () => {
    ...
    const { order } = orderStorage.addOrder({
      ...
      status: 'PREPARING',
      paymentStatus: 'PAID',
      paymentMethod: paymentMethod === 'DNUPAY' ? 'Ví DNU Pay' : paymentMethod === 'QRMOMO' ? 'QR MoMo' : 'Tiền mặt',
    ...
    dnuStore.addFinanceTransaction({
      ...
      paymentMethodLabel: paymentMethod === 'DNUPAY' ? 'Ví DNU Pay' : paymentMethod === 'QRMOMO' ? 'QR MoMo' : 'Tiền mặt',
